...
LSTMStrategy runs LSTM + Attention inference on a rolling tick buffer.
Tests verify the warmup guard, insufficient-feature guard, and that
the strategy never raises an exception during normal operation.
on_bar() is tested for correct delegation to on_tick logic.
"""
import asyncio
import math
import pytest
...
def make_strategy(warmup: int = 200, lookback: int = 60) -> LSTMStrategy:
    s = LSTMStrategy({"symbol": "SPY", "model_id": "test_lstm", "lookback": lookback})
...
def oscillating_price(i: int, base: float = 100.0) -> float:
    """Generate a realistic-ish price series using a sine wave."""
...
class TestFullBufferNoException:
    def test_no_exception_after_warmup(self):
        ...
        for i in range(100):
            result = run(s.on_tick(make_tick(oscillating_price(i), i)))
        # Either a signal dict or None — both are valid; must not raise
